...
- **Produto existente:** busca por SKU/título (com o mesmo `chave_alfabetica()` já usado nas outras abas) ou vê a lista "Só o que está faltando" (peso/EAN provisórios ou sem custo). Desde 19/09/2026 tem também "Só medidas fake" (só peso/comprimento/largura/altura, sem misturar EAN/custo).
- **Grade tem paginação (19-20/09/2026) — NUNCA assumir que "só os 12 primeiros aparecem" sem checar.** `LIMITE_GRADE=12` continua valendo por vez (widgets numéricos demais travam a edição, achado 03/08/2026), mas agora tem Anterior/Próximo em cima E embaixo da lista, então dá pra alcançar qualquer um dos N pendentes. Antes disso, um item em posição 29 ou 57 de 77 (ordem alfabética) nunca aparecia e não tinha como saber — foi o que gerou o caso real do LB00666/LB00689D (fake confirmado no Mercado Turbo, "sumido" da lista pra Bruna). Cada produto também virou um `st.container(border=True, key=f"pcard_{sku}")` — cor zebra via CSS (`.st-key-pcard_<sku>`) e o SKU em etiqueta preta/letra branca, em vez da linha de 12 colunas espremidas de antes.
